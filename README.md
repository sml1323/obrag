...
PARA 방법론 기반 프로젝트 관리를 제공하는 **통합 지식 관리 도구**
...
2. **Settings** 페이지에서 다음을 설정:
   - **Vault Path**: Obsidian Vault 경로 (절대 경로)
   - **LLM Provider / Model**: 사용할 LLM 선택 (OpenAI, Ollama)
   - **API Key**: 선택한 Provider의 API 키 입력
   - **Embedding Provider / Model**: 임베딩 모델 선택
3. 설정 완료 후 **Sync** 실행하여 노트 벡터화
