...
interface ParaDashboardProps {
  ...
  staleDays?: number;
  paraRoot: string | null;
  onParaRootChange: (path: string | null) => void;
}
    ...
    <div className="flex flex-col h-full">
      ...
      <header className="h-14 border-b border-border flex items-center justify-between px-4 bg-background/80 backdrop-blur-sm">
        <div className="flex items-center">
          ...
          {paraRoot && (
            <span className="ml-3 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium flex items-center gap-1">
      ...
      <FolderSelectModal
        ...
        onSelect={onParaRootChange}
        currentRoot={paraRoot}
      />
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>프로젝트 루트 폴더 선택</DialogTitle>
          <DialogDescription>
            PARA 프로젝트가 위치한 상위 폴더를 선택해주세요.
          </DialogDescription>
        </DialogHeader>
        
        <div className="min-h-[300px] max-h-[50vh] overflow-y-auto py-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-[300px]">
              <Character mood="loading" message="폴더 목록을 불러오는 중..." />
            </div>
          ) : (
            <div className="space-y-1">
              {nodes
                .filter((node) => node.is_dir && !node.path.startsWith("."))
                .map((node) => {
                  const isSelected = currentRoot === node.path;
                  return (
                    <button
                      key={node.path}
              ...
              {nodes.filter((node) => node.is_dir).length === 0 && (
                <div className="text-center py-12 text-muted-foreground">
                  <p>선택 가능한 폴더가 없습니다</p>
                </div>
              )}
