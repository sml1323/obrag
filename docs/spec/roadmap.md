...
- [x] **세션 관리**: 대화방(Session) 생성, 조회, 삭제 API (기본 CRUD 완료)
- [x] **대화 저장**: Chat API 호출 시 자동 저장 및 `history` 파라미터 대신 `session_id` 지원
- [x] **Frontend 연동 준비**: `GET /sessions?topic_id=...`, `GET /sessions/{id}`, `GET /sessions/{id}/messages` 엔드포인트 및 `DELETE` (Manual Cascade) 구현 완료
