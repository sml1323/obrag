...
Obsidian에 정리한 학습 노트가 쌓일수록 검색이 어려워지는 문제를 해결하기 위해, RAG와 AI를 활용한 의미 기반 검색 및 에빙하우스 망각곡선 기반 복습 시스템을 구축한다.
...
- **구조 분석:** `Project/` 폴더 하위의 디렉토리를 개별 프로젝트로 인식. 상위 폴더는 사용자가 직접 지정.
