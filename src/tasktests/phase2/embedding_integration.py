...
from config.models import OpenAIEmbeddingConfig, LocalEmbeddingConfig
...
class TestLocalEmbedder:
    ...
    def test_embed_raises_not_implemented(self):
        """embed() 호출 시 NotImplementedError 발생"""
        embedder = LocalEmbedder()
    ...
    def test_repr(self):
        ...
        assert "bge-m3" in repr_str
        assert "implemented=False" in repr_str
...
class TestEmbedderFactory:
    ...
    def test_unknown_provider_raises_error(self):
        """알 수 없는 provider 에러"""
        # 잘못된 provider로 config 생성 시도
        with pytest.raises(ValueError):
            # LocalEmbeddingConfig의 provider를 직접 변경하면 validation에서 에러
...
class TestOpenAIIntegration:
    ...
    def test_chromastore_with_openai(self, tmp_path):
        ...
        chunks = [
            MockChunk(text="Python is a programming language", metadata={"source": "test.md"}),
            MockChunk(text="Machine learning uses algorithms", metadata={"source": "test.md"}),
            MockChunk(text="Vector databases store embeddings", metadata={"source": "test.md"}),
        ]
    ...
    def test_chromastore_query_similarity(self, tmp_path):
        ...
        # 다양한 주제의 청크
        chunks = [
            MockChunk(text="FastAPI는 Python 웹 프레임워크입니다", metadata={"topic": "web", "source": "test.md"}),
            MockChunk(text="ChromaDB는 벡터 데이터베이스입니다", metadata={"topic": "db", "source": "test.md"}),
            MockChunk(text="React는 JavaScript UI 라이브러리입니다", metadata={"topic": "frontend", "source": "test.md"}),
            MockChunk(text="PostgreSQL은 관계형 데이터베이스입니다", metadata={"topic": "db", "source": "test.md"}),
        ]
