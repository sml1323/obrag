...
@pytest.fixture
def mock_app_state(mock_rag_chain, tmp_path):
    """Mock AppState."""
    chroma_store = MagicMock()
    chroma_store.get_stats.return_value = {"documents": 10, "collection": "test"}
    # persist_path를 실제 경로로 설정 — MagicMock이 str() 변환 시
    # "<MagicMock ...>" 문자열로 디렉토리가 생성되는 것을 방지
    chroma_store.persist_path = tmp_path / "test_chroma_db"
...
class TestChatStreamEndpoint:
    ...
    def test_stream_returns_event_stream(
        self, client, mock_rag_chain, mock_retrieval_result
    ):
    ...
    def test_stream_returns_sse_format(
        self, client, mock_rag_chain, mock_retrieval_result
    ):
        """SSE 형식 (data: ...) 확인."""
